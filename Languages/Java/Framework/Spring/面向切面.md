...
<aop:config>
	...
	<aop:aspect ref="minstrel">
		...
		<aop:pointcut id="embark" expression="execution(* *.embarkOnQuest(..)) "/>
         <!-- 前置通知：在 embarkOnQuest() 之前调用 minstrel 的 singBeforeQuest() 方法 -->
...
在切面类中使用 @PointCut 注解在方法上指定 pointcut，一般使用 execution 匹配方法
...
execution(* com.xyz.service.AccountService.*(..))  // 匹配所有 AccountService 接口的方法
execution(* com.xyz.service.*.*(..))  // 匹配 service 包下所有的方法
execution(* com.xyz.service..*.*(..))  // 匹配 service 包和它的子包下的方法
```
...
1. 声明切面类，必须在 POJO 类上使用该注解来声明该类是一个切面类且要和 @Component 一起使用，让 Spring 容器来管理 bean 或代理 bean
2. 声明切点标识
3. 在方法上使用通知
