...
使用反射的主要人员是工具构造者，而不是应用程序员。在 java 中用来支持其他特性，如对象序列化、JavaBean 

反射机制的默认行为受限于 java 的访问控制。如果一个 java 程序没有受到安全管理器的控制，就可以覆盖访问控制。*Class* 类和反射包一起对反射的提供支持，这些类型的对象由 jvm 在运行时创建，用以表示未知类里对应的成员，对于反射机制来说，.class 文件在编译时是不可获取的，是在运行时打开和检查 .class 文件
...
在泛型代码内部，无法获得任何有关泛型参数类型的信息，泛型使用擦除来实现，当使用泛型时，任何具体的类型信息都被擦出了。泛型不能用于显式地引用运行时类型的操作之中，例如转型、instanceof 操作和new 表达式
