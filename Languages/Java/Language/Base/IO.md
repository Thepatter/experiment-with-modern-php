...
反序列化时必须保证虚拟机能找的对应类的 `.class` 文件，否则会抛 *ClassNotFoundException* 异常
...
可以使用 **transient** 关键字来关闭字段的序列化

###### *Externalizable*

通过实现 *Externalizable*（任何信息都不会自动序列化）代替 *Serializable* 接口来对序列化过程进行控制。重写其 writeExternal（将来自对象的需要序列化的信息写入）和 readExternal（恢复对象数据）。这两个方法会在序列化和反序列化还原的过程中被自动调用。

对于一个 *Externalizable* 对象，所有普通的默认构造器都会被调用（包括字段定义时的初始化），然后调用 readExternal。

从一个 *Externalizable* 对象继承，通常需要调用基类版本的 *writeExternal* 和 *readExternal* 来为基类组件提供恰当的存储和恢复功能