...
将 Runnable 对象转变为工作任务的传统方式是把它提交给一个 *Thread* 构造器，调用 Thread.start() 方法为该线程执行必须的初始化操作，然后调用 Runnable.run() 方法，以便在这个新线程中启动该任务。
...
是指在程序运行的时候在后台提供一种通用服务的线程，并且这种线程并不属于程序中不可或缺的部分。因此，当所有的非后台线程结束时，程序也就终止了，只要有任何非后台线程还在运行，程序就不会终止。执行 main() 的就是一个非后台线程
...
一个线程可以在其他线程之上调用 join() 方法，其效果是等待一段时间直到第二个线程结束才继续执行。如果某个线程在另一个线程 t 上调用 t.join()，此线程将被挂起，直到目标线程 t 结束才恢复
