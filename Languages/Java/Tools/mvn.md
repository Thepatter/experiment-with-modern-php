...
maven 构建生命周期定义了一个项目构建根发布的过程。一个典型的 maven 构建（build）生命周期由以下几个阶段组成：

* validate

  validate 阶段处理验证项目，验证项目是否正确，所有必须信息是可用的
...
* package
  ...
  打包，创建 JAR/WAR 包（如在 pom.xml 中定义提及的包）
...
* install
  ...
  按照，按照打包的项目到本地仓库，以供其他项目使用

* deploy

  部署，拷贝最终的工程包到远程仓库中，以共享给其他开放人员和工程

为了完成默认生命周期，以上阶段（包括其他为列出阶段）将被按顺序执行。maven 有以下三个标准的生命周期：
...
一个插件目标代表一个特定的任务，这些目标可能被绑定到多个阶段或者无绑定。不绑定到任何构建阶段的目标可以在构建生命周期之外通过直接调用执行。这些目标的执行顺序取决于调用目标和构建阶段的顺序。
...
这里的 clean 阶段将会首先被执行，然后 dependency:copy-dependencies 目标会被执行，最终 package 阶段被执行

##### clean 生命周期

当执行 `mvn post-clean` 命令时，maven 调用 clean 生命周期，包含以下阶段：
...
在一个生命周期中，运行某个阶段的时候，它之前的所有阶段都会被执行，如果执行 `mvn clean` 将会执行 `pre-clean，clean`，如果运行 `mvn post-clean` ，将执行 `pre-clean，clean，post-clean`，可以在 clean 生命周期的任何阶段定义目标来修改这部分的操作行为。
...
![](./Images/maven构建build生命周期.png)

当一个阶段通过 maven 命令调用时，只有该阶段之前以及包括该阶段在内的所有阶段会被执行。不同的 maven 目标将根据打包类型（JAR/WAR/EAR），被绑定到不同的 maven 生命周期阶段

##### 命令行调用

在开放环境中，使用以下命令去构建、安装工程到本地仓库

```shell
mvn install
```

这个命令在执行 install 阶段前，按顺序执行了 default 生命周期（validate，compile，package），只需要调用最后一个阶段
...
这行命令也可用于多模块的情况下，即包含多个子项目的项目，maven 会在每一个子项目执行 clean 命令，然后再执行 deploy 命令
...
这里经常用到是 site 阶段和 site-deploy 阶段，用以生成和发布 maven 站点。执行以上 mvn 命令
