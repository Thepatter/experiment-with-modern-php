...
* `socket_connect()`
  ...
  `bool socket_connect (resource $socket, $string $address [, int $port = 0])`
   ...
   返回值：成功时返回 TRUE, 或者在失败时返回FALSE。 错误代码会传入 `socket_last_error()` ，如果将此参数传入 `socket_strerror()` 则可以得到错误的文字说明。
...
* `socket_create_pair()`
	...
	产生一对没有区别的 socket 到一个数组里，创建两个连接的和不可区分的套接字，并将它们存储在 fd, 此功能通常用于 IPC
...
* `socket_create()`
   ...
   `resource socket_create(int $domin, int $type, int $protocol)`
