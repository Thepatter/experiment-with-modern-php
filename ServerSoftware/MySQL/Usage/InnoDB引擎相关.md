...
![](../Images/Usage/InnoDB缓存池数据对象.png)

从 innodb 1.0.x 开始，允许有多个缓冲池实例，每个页根据哈希值平均分配到不同缓存池实例中。可以减少数据库内部的资源竞争，增加数据库的并发处理能力。通过参数 `innodb_buffer_pool_instances` 来进行配置，默认为 1，修改该配置需要将 `innodb_buffer_pool_size` 设置为大于 1 G
