...
|  load-on-startup  | 容器启动时自动加载 servlet，可以指定一个整数值来指定加载顺序，值小的 Servlet 优先加载，若没有指定值或值为负数，则由 web 容器决定如何加载，如果两个servlet 具有相同的 load-on-startup 值，则加载 Servlet 的顺序不能确定 |     一个可选     |
